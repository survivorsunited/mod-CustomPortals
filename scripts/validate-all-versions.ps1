...
foreach ($mcVersion in $allVersions) {
    ...
    if (-not $SkipServer) {
        ...
        try {
            ...
            # Configure server to use non-standard port to avoid conflicts
            # Use port 25570 + version offset (e.g., 1.21.8 -> 25578, 1.21.9 -> 25579)
            $portOffset = [int]($mcVersion -replace "1\.21\.", "")
            $serverPort = 25570 + $portOffset
            $serverProperties = @"
# Minecraft Server Properties
# Generated by validation script for version $mcVersion
...
motd=Custom Portals Test Server - $mcVersion
enable-rcon=false
"@
            $serverProperties | Out-File -FilePath "server.properties" -Encoding utf8 -Force
            Write-Host "🔌 Server configured to use port $serverPort (non-standard to avoid conflicts)" -ForegroundColor Cyan
            ...
            Write-Host "⏱️  Starting server on port $serverPort (timeout: ${ServerTimeout}s)..." -ForegroundColor Yellow
            ...
            Write-Host "🔌 Server running on non-standard port $serverPort to avoid conflicts" -ForegroundColor Gray
